...
public class ProductService {
  ...
  public String purchase (String userName,String productName,int quantity) {
    ...
    switch (updateProductTable) {
      case -1: return INVALID_PRODUCT;
      case 0: return ITEMS_UNAVAILABLE;
      case 1: {
  ...
  public String cancelOrder(int orderId) {
    ...
    int updateProductTable = table.update(cancelledOrder.productName,cancelledOrder.quantity, action);
    switch (updateProductTable) {
      case -1: return INVALID_PRODUCT;
      case 1: return String.format(CANCEL_SUCCESS, orderId);
      default: return "Table update Failed";
    }
  }
  ...
  if (searchOrders.isEmpty())
    ...
    for (Orders.Order order: searchOrders){
      String orderLine = String.format("%d %s %s %d\n", order.orderId, order.productName, order.quantity);
      orderResults.append(orderLine);
    }
    return orderResults.toString();
  }
  }
